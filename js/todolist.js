(() => {
    ...
    document.onreadystatechange = () => {
       ...
       Ajax.sendGetRequest(API_URL , {"id":" "} , MediaFormat.JSON , (Task) => loadTasks(Task), (code) => showError(1, 'la tarea no ha podido ser añidada'), true);
    };
    ...
    const addTask = (e) => {
        ...
        Ajax.sendPostRequest(API_URL, task, MediaFormat.JSON, (Task) => addTaskToList(task), (code) => showError(code, 'no ha sido añadida.'), true, MediaFormat.JSON);
        ...
        checkBox.onchange = (e) => {
            ...
            if (e.target.checked) {
                ...
                Ajax.sendPutRequest(`${API_URL}/${task.id}` , {status: TASK_STATUS.DONE}, MediaFormat.JSON, 
                    ...
                    (code) => showError(code, 'no fue modificado.'), true, MediaFormat.JSON);
    ...
    const removeTaskFromList = (id) => {
        ...
        ocument.getElementById(`task-${id}`).remove();
    };
